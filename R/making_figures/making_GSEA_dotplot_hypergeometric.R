...
cell.type.list <- c("HSC", "LMPP", "EMP", "MkP", "GMP")
...
fgsea.res.combined %>% 
  write_csv("data/differential_expression/VEXAS_MUT_vs_WT/gsea/fgsea_results_all_progenitors_3000_var_genes.csv")
...
fgsea.blocks = list(
  ...
  `GOBP_MYELOID_CELL_DIFFERENTIATION` = "Differentiation",
...
p.res <- fgsea.res.combined %>% 
  ...
  mutate(NES = if_else(padj < 0.2, NES, NA_real_)) %>%
  ...
  xlab("") + ylab("")
p.res
ggsave("figures/current_figure_drafts/RNA_gsea_dot_plot.pdf", plot = p.res, device = "pdf", dpi = 300, width = 5, height = 5, unit = "in")
