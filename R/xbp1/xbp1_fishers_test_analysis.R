...
celltype.list <- c("HSC", "EMP", "LMPP", "MkP")

donor.list <- rna.obj@meta.data %>% 
  filter(!is.na(Unspliced_XBP1)) %>% ## Remove cells without xbp1s info
  filter(cluster_celltype %in% celltype.list) %>% 
...
rna.obj@meta.data %>% filter(CellType %in% celltype.list) %>% 
  group_by(Donor, CellType, Genotype, ) %>% summarize(cell_count = n(), unspliced_total = sum(Unspliced_XBP1, na.rm = T), spliced_total = sum(Spliced_XBP1, na.rm = T)) %>% 
  filter(Genotype != "NA") %>% 
  filter(cell_count > 25) %>% 
  group_by(Donor, CellType) %>% 
  filter(n() > 1) %>% 
  write_csv("data/xbp1_splicing/xbp1_read_counts.csv")
...
do.call(rbind, fishers.test.results) %>% 
  write_csv("data/xbp1_splicing/xbp1_fishers_test_results.csv")


## Run fisher's test for progenitors combined
fishers.test.results.all.progenitors <- do.call(rbind, lapply(donor.list, function(donor.name) {run_fishers_test(donor.name = donor.name, celltype.list = c("HSC", "EMP", "LMPP", "MkP"))}))
...
fishers.test.results.plots <- lapply(names(fishers.test.results), function(celltype) {
  fishers.test.results[[celltype]] %>%
    ...
    # geom_text(aes(label = scales::scientific(p_value, digits = 3)), y = 3.5) +
    ...
    scale_fill_manual(values = c(`TRUE` = "grey40", `FALSE` = "grey80")) +
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1), legend.position = "bottom") +
    ggtitle("XBP1 Splicing Ratio, MUT vs. WT cells", subtitle = "HSCs")
})
names(fishers.test.results.plots) <- names(fishers.test.results)

## save test results
fishers.test.results$HSC %>% write_csv("fishers_test_outcome_hscs_by_donor.csv")


## Plot HSCs separately
p.hsc <- fishers.test.results.plots$HSC + theme(legend.position = "right")
p.hsc
ggsave("figures/current_figure_drafts/XBP1_fishers_test_bar_plot_split_by_patient.pdf", plot = p.hsc, device = "pdf", dpi = 300, width = 4, height = 3, unit = "in")
...
p.fishers.test
ggsave("figures/current_figure_drafts/XBP1_fishers_test_bar_plot_split_by_patient_HSPCs.pdf", plot = p.fishers.test, device = "pdf", dpi = 300, width = 4, height = 3, unit = "in")

## save test results
fishers.test.results.all.progenitors %>% write_csv("fishers_test_outcome_hspcs_by_donor.csv")
